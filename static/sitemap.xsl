...
<head>
	...
	<style type="text/css">body{font-family:"Lucida Grande","Lucida Sans Unicode",Tahoma,Verdana,sans-serif;font-size:13px}#header,#footer{padding:2px;margin:10px;font-size:10pt;color:gray}a{color:blue}td{font-size:12px}th{text-align:left;padding-right:60px;font-size:11px}tr.high{background-color:whitesmoke}#footer img{vertical-align:middle}</style>
</head>
<body>
	<h1>XML Sitemap</h1>
	<div id="header">
		<p>This is the site map provided by Shangxian technology for customers, which is convenient for Google, Baidu and other search engines to better find your website.</p>
	</div>
	<div id="content">
...
<xsl:for-each select="sitemap:urlset/sitemap:url">
			<tr><xsl:if test="position() mod 2 != 1"><xsl:attribute  name="class">high</xsl:attribute></xsl:if>
				...
				<td><xsl:if test="sitemap:changefreq"><xsl:value-of select="concat(sitemap:changefreq,'')"/></xsl:if></td>
